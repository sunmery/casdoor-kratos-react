...
import (
	...
	pb "backend/api/casdoor/v1"
)

type CasdoorService struct {
	pb.UnimplementedCasdoorServer
	cc *biz.CasdoorUsecase
}

func NewCasdoorService(cc *biz.CasdoorUsecase) *CasdoorService {
	return &CasdoorService{cc: cc}
}

func (s *CasdoorService) Signin(ctx context.Context, req *pb.SigninRequest) (*pb.SigninReply, error) {
	result, err := s.cc.Signin(ctx, &biz.SigninRequest{
		State: req.State,
		Code:  req.Code,
	})
